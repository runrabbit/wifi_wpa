...
#define SIOCIWP2PSET SIOCIWFIRSTPRIV + 0x10
...
{
	char *param;
	wpa_printf(MSG_ERROR, "ygg cmd p2p_ctrl_set 1 '%s'", cmd);

	if (wpa_s->global->p2p == NULL){
		wpa_printf(MSG_DEBUG, "ygg cmd p2p not support");
		return -1;
		}
wpa_printf(MSG_ERROR, "ygg cmd  p2p_ctrl_set 2 '%s'", cmd);
...
wpa_driver_wext_p2p_set_enable();
	wpa_printf(MSG_ERROR, "ygg p2p set enable");

		return 0;
	}
	wpa_printf(MSG_ERROR, "CTRL_IFACE: Unknown P2P_SET field value '%s'",
		   cmd);

	return -1;
}

//ygg porting from iwpri
#if 0
...
printf("args %s  %s count  %d  ifname %s \n",*args,*(args+1),count,ifname);
//root@android:/ # iwpriv wlan0 p2p_set enable=1                                 
//args p2p_set  enable=1 count  2  ifname wlan0 
...
//ygg porting from iwpri end

#endif
//ygg 
int wpa_driver_wext_p2p_set_enable(struct wpa_driver_wext_data *drv,
				   int idx, u32 value)
{
	
	int ret = 0;
char **argv={"p2p_set","enable=1"};
	
	// set_private(drv->ioctl_sock, argv, 2, "wlan0");
//SIOCIWP2PSET
					 ...
					 char *buf, size_t *resp_len)
{
	...
	int ctrl_rsp = 0;
	int reply_len;
wpa_printf(MSG_DEBUG, "  MSG_DEBUG wpa_supplicant_ctrl_iface_process buf=%s ", buf);
wpa_printf(MSG_EXCESSIVE, "MSG_EXCESSIVE wpa_supplicant_ctrl_iface_process buf=%s ", buf);
wpa_printf(MSG_ERROR, " MSG_ERROR wpa_supplicant_ctrl_iface_process buf=%s ", buf);
#ifdef CONFIG_P2P
wpa_printf(MSG_ERROR, " CONFIG_P2P");
wpa_printf(MSG_EXCESSIVE, " CONFIG_P2P");
#endif
...
wpa_printf(MSG_ERROR, "ygg cmd wpa_supplicant_global_iface_add 1 '%s'", cmd);
