...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
	...
	char buf[200];
#endif
wpa_printf(MSG_ERROR,"ygg enter  wpas_p2p_init wpa_s->drv_flags %d WPA_DRIVER_FLAGS %d ",wpa_s->drv_flags,WPA_DRIVER_FLAGS_P2P_CAPABLE );
	/*if (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_P2P_CAPABLE))
		return 0;*/
wpa_printf(MSG_ERROR,"ygg   wpas_p2p_init " );
wpa_printf(MSG_DEBUG, " %s  %d before call ",__FUNCTION__,__LINE__);
	...
	if (global->p2p)
		return 0;
wpa_printf(MSG_ERROR,"ygg   wpas_p2p_init  rellay begain" );
	if (wpa_s->drv_flags & WPA_DRIVER_FLAGS_P2P_MGMT) {
