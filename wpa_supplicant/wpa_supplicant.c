				     ...
				     struct wpa_interface *iface)
{
	...
	struct wpa_driver_capa capa;
wpa_printf(MSG_ERROR, "ygg wpa_supplicant_init_iface");
	...
	wpa_printf(MSG_ERROR, "ygg Initializing interface '%s' conf '%s' driver "
		   "'%s' ctrl_interface '%s' bridge '%s'", iface->ifname,
		   iface->confname ? iface->confname : "N/A",
		   iface->driver ? iface->driver : "default",
		   iface->ctrl_interface ? iface->ctrl_interface : "N/A",
...
next_driver:
	if (wpa_supplicant_set_driver(wpa_s, driver) < 0)
		return -1;
  wpa_printf(MSG_ERROR,"ygg before wpa_drv_init ");
	wpa_s->drv_priv = wpa_drv_init(wpa_s, wpa_s->ifname);
	wpa_printf(MSG_ERROR,"ygg after wpa_drv_init ");
	...
	wpa_printf(MSG_DEBUG, " %s  %d before call ",__FUNCTION__,__LINE__);
...
wpa_printf(MSG_DEBUG, " %s  %d before call wpa_drv_init",__FUNCTION__,__LINE__);
...
wpa_printf(MSG_ERROR,"ygg before wpa_supplicant_driver_init ");
	if (wpa_supplicant_driver_init(wpa_s) < 0)
		return -1;
wpa_printf(MSG_ERROR,"ygg after wpa_supplicant_driver_init ");
#ifdef CONFIG_TDLS
...
wpa_printf(MSG_ERROR,"ygg before wpas_p2p_init ");
#ifdef CONFIG_P2P
wpa_printf(MSG_ERROR,"ygg before wpas_p2p_init 1");
						 ...
						 struct wpa_interface *iface)
{
	...
	struct wpa_ssid *ssid;
wpa_printf(MSG_ERROR, "ygg wpa_supplicant_add_iface");
