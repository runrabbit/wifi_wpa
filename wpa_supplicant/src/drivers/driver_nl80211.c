...
#ifdef ygg_rm_ANDROID_BRCM_P2P_PATCH
static void mlme_event_deauth_disassoc(struct wpa_driver_nl80211_data *drv,
				  enum wpa_event_type type,
				  const u8 *frame, size_t len);
			 ...
			 void *valid_data)
{
	struct nl_cb *cb;
	int err = -ENOMEM;
wpa_printf(MSG_DEBUG, " ygg enter  %s  %d  call ",__FUNCTION__,__LINE__);
 ...
 out:
	nl_cb_put(cb);
	nlmsg_free(msg);
	wpa_printf(MSG_DEBUG, " ygg exit   %s  %d  err %d  ",__FUNCTION__,__LINE__,err);
	return err;
}
...
wpa_printf(MSG_DEBUG, " ygg enter  %s  %d   ",__FUNCTION__,__LINE__);
	...
	if (send_and_recv_msgs(drv, msg, wiphy_info_handler, info) == 0)
		{
		wpa_printf(MSG_DEBUG, " ygg enter  %s  %d  exit success ",__FUNCTION__,__LINE__);
		return 0;
		}
	msg = NULL;
nla_put_failure:
	nlmsg_free(msg);
	wpa_printf(MSG_DEBUG, " ygg enter  %s  %d  exit failure ",__FUNCTION__,__LINE__);
	return -1;
}
...
wpa_printf(MSG_DEBUG, " ygg eixt %s  %d  success ",__FUNCTION__,__LINE__);
	return 0;
}
...
wpa_driver_nl80211_finish_drv_init(struct wpa_driver_nl80211_data *drv)
{
	...
	netlink_send_oper_ifla(drv->netlink, drv->ifindex,
			       1, IF_OPER_DORMANT);
#endif /* HOSTAPD */
wpa_printf(MSG_DEBUG, " ygg before call wpa_driver_nl80211_capa");
	if (wpa_driver_nl80211_capa(drv))
		return -1;
wpa_printf(MSG_DEBUG, " ygg after call wpa_driver_nl80211_capa");
...
#endif
#ifdef ANDROID
	//ygg_rm .driver_cmd = wpa_driver_nl80211_driver_cmd,
#endif
};
